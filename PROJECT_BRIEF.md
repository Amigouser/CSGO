# TomskGG — Киберспортивная платформа Томска

## Краткое описание
Клон сайта dotaint.kg — платформа для турниров по Dota 2 и CS2. Перенесено на Томск вместо Кыргызстана. Название: **TomskGG**.
...
## Контекст для AI
Это киберспортивная платформа для города Томск (Россия). Пользователи регистрируются через Steam, участвуют в турнирах по Dota 2 и CS2, получают рейтинг (ELO), достижения, могут предсказывать результаты матчей. Сайт адаптивный, с тёмной/светлой темой, тремя языками, PWA-поддержкой.
